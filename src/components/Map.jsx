...
import { getBoundingBoxFromPolygon, getMapGraph } from "../services/MapService";
...
function Map() {
    ...
    async function mapClick(e, info, radius = null) {
        ...
        if(info.rightButton || placeEnd) {
            ...
            // Search locally downloaded nodes to avoid API call
            for (const [id, n] of state.current.graph.nodes.entries()) {
                const len = Math.hypot(n.latitude - e.coordinate[1], n.longitude - e.coordinate[0]);
                if (len < minLength) {
                    minLength = len;
                    node = { id: n.id, lat: n.latitude, lon: n.longitude };
                }
            }
        ...
        const clickLat = e.coordinate[1];
        const clickLon = e.coordinate[0];
        const circle = createGeoJSONCircle([clickLon, clickLat], radius ?? settings.radius);

        // Fetch nodes inside the radius directly!
        getMapGraph(getBoundingBoxFromPolygon(circle)).then(graph => {
            state.current.graph = graph;

            let closestNode = null;
            let minStartLen = Number.MAX_VALUE;
            // Snapping to the nearest valid graph node purely in local memory
            for (const [id, n] of graph.nodes.entries()) {
                const len = Math.hypot(n.latitude - clickLat, n.longitude - clickLon);
                if (len < minStartLen) {
                    minStartLen = len;
                    closestNode = { id: n.id, lat: n.latitude, lon: n.longitude };
                }
            }
            ...
            graph.startNode = graph.getNode(closestNode.id);
            ...
            ui.current.showSnack("Error fetching graph data.");
            clearTimeout(loadingHandle);
            setLoading(false);
            console.error(err);
        });
    ...
    // Progress animation by one step — runs the algorithm N times, but advances time only once
    function animateStep(deltaTime) {
        const speed = speedRef.current;

        // Advance algorithm N steps per frame based on speed
        for (let i = 0; i < speed; i++) {
            if (state.current.finished) break;
        ...
        // Found end but waiting for route trace animation to end
        if(state.current.finished && !animationEnded) {
            // Render route differently for bidirectional
            if(settings.algorithm === "bidirectional") {
                if(!traceNode.current) traceNode.current = state.current.endNode; // use meeting node
                const parentNode = traceNode.current?.parent;
                updateWaypoints(parentNode, traceNode.current, "route", Math.max(Math.log2(speed), 1));
                traceNode.current = parentNode ?? traceNode.current;

                if(!traceNode2.current) {
                    traceNode2.current = state.current.endNode;
                    if (traceNode2.current) traceNode2.current.parent = traceNode2.current.prevParent;
                }
                const parentNode2 = traceNode2.current?.parent;
                updateWaypoints(parentNode2, traceNode2.current, "route", Math.max(Math.log2(speed), 1));
        ...
        // Advance visual clock once per frame (not N times)
        if (!animationEnded) {
            setTime(prevTime => prevTime + deltaTime * playbackDirection);
        }

        // Playback scrubbing
        if(animationEnded && playbackOn) {
    ...
    // Animation callback — one rAF tick = one frame
    function animate(newTime) {
        if (previousTimeRef.current != null) {
            const deltaTime = newTime - previousTimeRef.current;
            animateStep(deltaTime);
        }
